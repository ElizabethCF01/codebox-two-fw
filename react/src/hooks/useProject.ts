...
import { Project, ProjectRequest, ProjectResponse, ProjectsResponse } from "../interfaces/project"
    ...
    const getProjectList = async (page = 1, query = '') => {
        ...
        try {
          ...
          if (response) {
            ...
            if (response.data.meta.pagination.pageCount === response.data.meta.pagination.page) {
              hasMore = false
            }
          }
        } catch (error) {
      ...
      const getProjectById = async (id: string) => {
        ...
        try {
          const response = await axiosClient.get<ProjectResponse>(`projects/${id}?populate=*`)
      ...
      const createProject = async (request: ProjectRequest) => {
        let allOk = false
        try {
          const response = await axiosClient.post<ProjectResponse>(`projects/`, request)
